...
def nn_forward_pass(params, X):
    ...
    # Store the result in the scores variable, which should be an tensor of shape (N, C).
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    # This should include both the data loss and L2 regularization for W1 and W2. 
    ...
    exp_total = exp_score.sum(dim=1, keepdim=True)
    softmax_score = exp_score / exp_total

    # compute loss: use the log of the correct class probabilities
    correct_class = softmax_score[torch.arange(N), y]
    ...
    # output layer: the gradient of loss with respect to the scores (output)
...
def nn_get_search_params():
  ...
  # different hyperparameters to achieve good performance with softmax classifier. 
  learning_rates = [1e-4, 1e-3, 1e-2, 1e-1, 1]
  hidden_sizes = [8, 16, 32, 64, 128]
  regularization_strengths = [0, 1e-1, 1e-2, 1e-3]
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  # network.
  learning_rates, hidden_sizes, regularizations, decays = nn_get_search_params()
  for lr in learning_rates:
    for sz in hidden_sizes:
      for reg in regularizations:
        for dc in decays:
          net = TwoLayerNet(3*32*32, sz, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
          stat = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'], 
                            num_iters=3000, batch_size=1000, learning_rate=lr, learning_rate_decay=dc, reg=reg)
          pred = net.predict(data_dict['X_val'])
          accuracy = (pred == data_dict['y_val']).double().mean().item()
