...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      train_vector = x_train[i].view(-1)
      test_vector = x_test[j].view(-1)
      dists[i, j] = torch.sum((train_vector - test_vector) ** 2)
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  dists = x_train.new_zeros(num_train, num_test)
  for i in range(num_train):
    train = x_train[i].view(1, -1)
    test = x_test.view(num_test, -1)
    dists[i] = torch.sum((train - test) ** 2, dim=1)
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  train_square = (x_train_flat ** 2).sum(dim=1).unsqueeze(1)
  test_square = (x_test_flat ** 2).sum(dim=1).unsqueeze(0)
  cross = torch.mm(x_train_flat, x_test_flat.t())
  dists = train_square + test_square - cross * 2
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  # TODO: Implement this function. You may use an explicit loop over the test samples.
  # Hint: Look up the function torch.topk
  _, indices = torch.topk(dists, k, dim=0, largest=False)
  for i in range(num_test):
    y_pred[i], _ = torch.mode(y_train[indices[:, i]])
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      x_train_set = torch.cat(x_train_folds[0:i] + x_train_folds[i+1:num_folds])
      y_train_set = torch.cat(y_train_folds[0:i] + y_train_folds[i+1:num_folds])
